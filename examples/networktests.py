...
vnets = mcazurerm.list_vnets(access_token, subscription_id)
for vnet in vnets['value']:
    print(vnet['name'] + ', ' + vnet['location'])

# list NICs in subscription
print('\nNICs in subscription:')
nics = mcazurerm.list_nics(access_token, subscription_id)
...
nics = mcazurerm.list_nics_rg(access_token, subscription_id, resource_group)
for nic in nics['value']:
    print(json.dumps(nic, sort_keys=False, indent=2, separators=(',', ': ')))
    # print(nic['name'])
...
lbs = mcazurerm.list_load_balancers(access_token, subscription_id)
for lb in lbs['value']:
    print(lb['name'] + ', ' + lb['location'])

# list load balancers in resource group
print('\nLoad Balancers in resource group: ' + resource_group)
lbs = mcazurerm.list_load_balancers_rg(access_token, subscription_id, resource_group)
for lb in lbs['value']:
    print(lb['name'] + ', ' + lb['location'])
...
lb = mcazurerm.get_load_balancer(access_token, subscription_id, resource_group, lb_name)
...
ips = mcazurerm.list_public_ips(access_token, subscription_id, resource_group)
...
# get a public ip 
ip = mcazurerm.get_public_ip(access_token, subscription_id, resource_group, resource_group + 'pip')
...
usage = mcazurerm.get_network_usage(access_token, subscription_id, location)
